// This is from Noctalia by Lycos - commit 4937382001c6e1c58c87cac050747aa95623dda8
import QtQuick
    ...
    Repeater {
        ...
        delegate: Item {
            ...
            PanelWindow {
                ...
                // TODO: Watch for changes in notification monitors setting
                // Connections {
                ...
                Column {
                    ...
                    Repeater {
                        ...
                        delegate: Rectangle {
                            ...
                            property Notification rawNotification: model.rawNotification
                            ...
                            MouseArea {
                                ...
                                onClicked: {
                                    dismissAnimation.start();
                                    if (rawNotification) {
                                        if (rawNotification.actions.length !== 0) {
                                            const actions = rawNotification.actions;
                                            for (var i = 0; i < actions.length; i++) {
                                                const action = actions[i];
                                                if (action.identifier !== "default") {
                                                    console.log("[NotificationPopup] unhandled action from app", rawNotification.appName, "with summary", rawNotification.summary, "action identifier", action.identifier);
                                                    rawNotification.dismiss();
                                                } else {
                                                    action.invoke();
                                                    console.log("[NotificationPopup] activated the default action");
                                                }
                                            }
                                        } else {
                                            rawNotification.dismiss();
                            ...
                            ParallelAnimation {
                                id: dismissAnimation
                                property int dur: 150
                                NumberAnimation {
                                    target: notificationDelegate
                                    property: "opacity"
                                    to: 0
                                    duration: dismissAnimation.dur
